...
**文件**：`io/profiles.py`, `scripts/make_profiles.py`
**函数**
...
**运行**：`python -m tdnmpc.scripts.make_profiles --out data/profiles.csv`
...
**运行**：`python -m tdnmpc.scripts.build_dso --case ieee33 --out data/ieee33.pkl`
...
**运行**：`python -m tdnmpc.scripts.build_tso --n-bus 30`
...
**运行**：`python -m tdnmpc.scripts.run_admm --T 12`
...
**运行**：`python -m tdnmpc.scripts.run_experiment --cfg cfg/demo.yaml`
...
**文件**：`coord/admm.py`（多从属）、`scripts/run_experiment.py`（增加 `--seeds`）
**函数**

```python
def make_multi_dso(n: int) -> list["pp.pandapowerNet"]
def run_batch(cfg: Path, seeds: list[int]) -> pd.DataFrame
```
